...
void framebf_init(int s_width, int s_height) 
{ 
    ...
    mBuf[5] = s_width; // Value(width) 
    mBuf[6] = s_height; // Value(height) 
...
void drawImageARGB32(int cur_x, int cur_y, int i_width, int i_height,int s_width, int s_height, const unsigned long* image_bit){ //x, y for image position, width height for frame & image size
    ...
    int xlim = i_width < s_width? i_width : s_width; // check which ever size is smaller, image or frame and take the smaller one
    int ylim = i_height <s_height? i_height : s_height;
